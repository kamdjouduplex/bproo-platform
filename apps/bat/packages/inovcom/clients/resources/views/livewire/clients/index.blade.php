...
<div>
    <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        <a class="btn btn-primary" href="{{ route('tenant.clients.create', ['tenant' => $tenantCode]) }}">
            + {{ __('Nouveau') }}
        </a>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full border-collapse text-[12px]">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors" wire:key="client-{{ $client->id }}">
                        <td class="px-4 py-2.5 font-mono text-xs text-indigo-600">{{ $client->code }}</td>
                        <td class="px-4 py-2.5 font-medium text-slate-800">{{ $client->name }}</td>
                        <td class="px-4 py-2.5 text-slate-500">{{ $client->type === 'company' ? __('Entreprise') : __('Particulier') }}</td>
                        ...
                        <td class="px-4 py-2.5">
                            <div class="flex items-center gap-1">
                                <a class="table-action table-action-edit"
                                   href="{{ route('tenant.clients.edit', ['tenant' => $tenantCode, 'client' => $client->id]) }}"
                                   title="{{ __('Modifier') }}">
                                ...
                                <button type="button" class="table-action table-action-delete"
                                        ...
                                        wire:confirm="{{ __('Supprimer ce client ?') }}"
                                        title="{{ __('Supprimer') }}">
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-slate-400 text-sm">{{ __('Aucun client.') }}</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
