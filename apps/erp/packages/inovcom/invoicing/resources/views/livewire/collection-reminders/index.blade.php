        ...
        <div class="table-toolbar">
            ...
            <form wire:submit.prevent="applyFilters" class="collection-reminders-toolbar" role="search">
                ...
                @if ($groups->isNotEmpty())
                    ...
                    @if ($canExport)
                        <a class="btn btn-export btn-export--pdf btn-sm" href="{{ $pdfUrl }}">
                            <x-file-type-icon format="pdf" class="btn-export__glyph" />
                            <span class="btn-export__label">PDF</span>
                        </a>
                        <button type="button" class="btn btn-export btn-export--excel btn-sm" wire:click="exportExcel">
                        ...
                        <tbody>
                            @foreach ($group['invoices'] as $row)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if (($row['amount_due_now'] ?? null) !== null)
                                            <strong style="color:#b45309;">{{ fmt_money((float) $row['amount_due_now']) }}</strong>
                                        @else
                            ...
                            <tr class="collection-reminders-total-row">
                                ...
                                <td colspan="3"><strong>{{ fmt_money($group['total_balance']) }}</strong></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
