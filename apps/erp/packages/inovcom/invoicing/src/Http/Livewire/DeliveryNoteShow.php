...
class DeliveryNoteShow extends Component
{
    ...
    public function updatedShowPrices(bool $value): void
    {
        ...
        $this->persistPrintOptionsQuietly();
    }

    public function updatedShowDiscounts(bool $value): void
    {
        if ($value && !$this->show_prices) {
    ...
    /**
     * Enregistre prix/remises dès le clic sur les cases (sans message flash).
     */
    private function persistPrintOptionsQuietly(): void
    {
        if (!$this->canEditPrintOptions()) {
            return;
        }
    ...
    private function canEditPrintOptions(): bool
    {
        if (!$this->canCreate()) {
            return false;
        }
        ...
        return $note && ($note->isDraft() || $note->isConfirmed());
    }
