        ...
        @if ($canEdit)
            <div style="margin-bottom:20px;padding:14px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;">
                ...
                <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap;">
                    <button type="button" class="btn btn-secondary btn-sm" wire:click="savePrintOptions">Enregistrer options d'impression</button>
                </div>
            </div>
        @endif
                ...
                <tbody>
                    @foreach ($deliveryNote->lines as $index => $line)
                        ...
                        <tr>
                            ...
                            @if ($show_prices && $row)
                                <td>{{ fmt_num((float) $row['unit_price'], 2) }}</td>
                                @if ($show_discounts)
                                    @php
                                        ...
                                        $puNet = (float) ($row['unit_price_net'] ?? 0);
                                    @endphp
                                    <td>{{ $discountLabel !== '—' ? $discountLabel : '' }}</td>
                                    <td>{{ fmt_num($puNet, 2) }}</td>
                                @endif
