...
class InvoiceForm extends Component
{
    ...
    private function loadFromDeliveryNote(int $deliveryNoteId): void
    {
        ...
        $this->cart = $note->lines->map(function ($dnLine) use ($quotationLinesById) {
            ...
            $lineDiscountAmount = $qLine
                ? (float) ($qLine->line_discount ?? 0)
                : ($iLine ? (float) ($iLine->line_discount ?? 0) : 0.0);
            $qty = (float) $dnLine->quantity;
            $puNet = max(0, $unitPrice - $lineDiscountAmount);
    ...
    public function updatedCart(): void
    {
        foreach ($this->cart as $i => $row) {
            ...
            $discount = $this->resolveLineDiscountAmount($price, $row);
            $puNet = max(0, $price - $discount);
    ...
    public function save(bool $andIssue = false): void
    {
        ...
        $lines = array_map(function ($row) {
            ...
            $mode = ($row['line_discount_mode'] ?? 'amount') === 'percent' ? 'percent' : 'amount';
            $input = max(0, (float) ($row['line_discount'] ?? 0));
            $discountAmount = $this->resolveLineDiscountAmount($price, $row);
    ...
    public function render()
    {
        $invoice = $this->invoiceId
            ? Invoice::with(['client', 'quotation', 'lines'])->find($this->invoiceId)
            : null;
    ...
    /**
     * @return array{line_discount_mode: string, line_discount: string}
     */
    ...
    {
        $lineMode = 'amount';
        $lineInput = (float) ($line->line_discount ?? 0);
        $table = $line->getTable();
        ...
        if (Schema::connection('tenant')->hasColumn($table, 'line_discount_mode')) {
            ...
            if (Schema::connection('tenant')->hasColumn($table, 'line_discount_input')
                && $line->line_discount_input !== null) {
                $lineInput = (float) $line->line_discount_input;
            } elseif ($lineMode === 'percent' && (float) ($line->unit_price ?? 0) > 0 && $lineInput > 0) {
                $lineInput = round($lineInput / (float) $line->unit_price * 100, 4);
            }
        }

        return [
            'line_discount_mode' => $lineMode,
            'line_discount' => (string) ($lineInput > 0 ? fmt_num_plain($lineInput) : '0'),
        ];
    }
