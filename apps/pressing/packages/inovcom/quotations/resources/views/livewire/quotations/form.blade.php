            ...
            <div class="page-actions" style="flex-wrap:wrap; gap:8px;">
                ...
                @if ($canValidate && $quotation->status === 'sent')
                    ...
                    @if ($savedPurchaseOrder === '')
                        <span class="field-hint" style="align-self:center;">Enregistrez le n° demande achat avant acceptation.</span>
                    @elseif ($unsavedPurchaseOrder)
                        <span class="field-hint" style="align-self:center;color:#b45309;">N° demande achat modifié — enregistrez avant acceptation.</span>
                    @endif
                    <button type="button" class="btn btn-primary btn-sm" wire:click="setStatus('accepted')" wire:confirm="Le client a accepté ce devis ?"
        ...
        <section class="card" style="margin-bottom: 16px;">
            ...
            <div class="form-grid">
                ...
                <div class="form-group @error('customer_purchase_order') form-group--invalid @enderror">
                    ...
                    >
                    @if ($quotation && $quotation->status === 'sent')
                        <p class="field-hint">Obligatoire avant de marquer le devis comme accepté (enregistrez après saisie).</p>
                    @endif
                    @error('customer_purchase_order') <span class="field-error">{{ $message }}</span> @enderror
                </div>
                <div @class([
                    'form-group',
            ...
            <table>
                ...
                <tbody>
                    ...
                    @php
                        ...
                        $lineMode = ($row['line_discount_mode'] ?? 'amount') === 'percent' ? 'percent' : 'amount';
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="quote-line-discount">
                                <select class="input input-sm quote-line-discount__mode" wire:model.live="cart.{{ $index }}.line_discount_mode" @disabled(!$canEdit)>
                                ...
                                <input
                                    ...
                                    step="0.01"
                                    min="0"
                                    @if (($row['line_discount_mode'] ?? 'amount') === 'percent') max="100" @endif
