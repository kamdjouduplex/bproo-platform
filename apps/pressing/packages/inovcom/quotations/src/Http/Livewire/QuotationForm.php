...
class QuotationForm extends Component
{
    ...
    public function save(): void
    {
        ...
        $lines = array_map(function ($row) {
            ...
            $payload = [
                ...
                'line_discount' => $this->resolveLineDiscountAmount((float) $row['unit_price'], $row),
    ...
    public function setStatus(string $status): void
    {
        ...
        if (!$quotation->isEditable() && $status !== 'accepted') {
            session()->flash('error', 'Ce devis ne peut plus changer de statut.');
            return;
        }
        ...
        if ($status === 'accepted') {
            ...
            if ($formPo !== '' && $formPo !== $savedPo) {
                session()->flash('error', 'Enregistrez le devis avant de marquer accepté (n° demande achat non sauvegardé).');
                return;
            }

            if ($savedPo === '') {
                session()->flash('error', 'Le n° demande achat est obligatoire. Enregistrez le devis avec ce numéro avant de marquer accepté.');
                return;
            }
        }
