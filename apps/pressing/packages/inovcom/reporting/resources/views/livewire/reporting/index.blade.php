...
<div class="reporting">
    ...
    {{-- Filtre période juste sous les onglets --}}
        ...
        <div class="reporting-filters__row">
            ...
            <div class="reporting-filters__controls">
                ...
                @elseif ($period === 'custom')
                    ...
                    <button type="button" class="btn btn-secondary btn-sm" wire:click="applyPeriod">Appliquer</button>
                @endif
            </div>
            <span class="reporting-filters__period-label">{{ $periodLabel }}</span>
        </div>
    </section>
    ...
    @if ($activeTab === 'general')
        <section class="reporting-cards" aria-label="Indicateurs clés">
            @foreach ($summaryCards as $card)
                <div class="reporting-card {{ $card['status'] ? 'reporting-card--' . $card['status'] : '' }}">
        ...
        <div class="reporting-overview-grid">
            <section class="reporting-panel">
                <h2 class="reporting-panel__title">Vue d’ensemble — {{ $periodLabel }}</h2>
                <div class="reporting-panel__body">
                    ...
                    <div class="reporting-quick-links">
                        <button type="button" class="btn btn-secondary btn-sm" wire:click="selectTab('finances')">Détail finances</button>
                        ...
                        <div class="reporting-stat reporting-stat--{{ $ind['status'] }}" style="margin-bottom:10px;">
                            ...
                            <span class="reporting-stat__value">
                                ...
                                @else
                                    ...
                                    @if ($ind['desc']) <span class="reporting-muted">({{ $ind['desc'] }})</span> @endif
                                @endif
                            </span>
                        </div>
                    @endforeach
            ...
            @if ($hasQuotations || $hasInvoicing)
                <section class="reporting-panel">
                    ...
                    <div class="reporting-panel__body">
                        ...
                        @endif
                        <div class="reporting-quick-links">
                            <button type="button" class="btn btn-secondary btn-sm" wire:click="selectTab('commercial')">Voir le détail</button>
                        </div>
                    </div>
                </section>
            @endif
                                ...
                                datasets: [
                                    {
                                        label: 'CA',
                                        data: salesData,
                                        backgroundColor: salesData.map((v, i) => i === maxIdx ? '#1d4ed8' : i === minIdx ? '#93c5fd' : '#2563eb'),
                                        borderColor: '#1e40af', borderWidth: 1.5, order: 2
                                    },
                                    {
                                        label: 'Bénéfice',
                                        data: benefitData,
        ...
        <section class="reporting-panel reporting-explorer">
            ...
            <div class="reporting-panel__body">
                ...
                <div class="reporting-explorer__presets" role="group" aria-label="Scénarios rapides">
                    @foreach ([
                        'ca_client' => ['CA client', 'Chiffre d’affaires par entreprise'],
                        'top_products' => ['Top CA', 'Marchandises qui rapportent le plus'],
                        'top_products_qty' => ['Top qté', 'Marchandises les plus vendues'],
                        'ventes_direct' => ['Ventes', 'Liste détaillée Vente Direct'],
                        'factures' => ['Factures', 'CA Facture HT / TTC / solde'],
                        'ca_ht_tva' => ['TVA', 'Séparation CA HT et TVA'],
                        'stock_low' => ['Stock bas', 'Articles sous le seuil'],
                        'stock_out' => ['Ruptures', 'Articles à 0'],
                    ] as $presetKey => $preset)
                ...
                <div class="reporting-explorer__toolbar">
                    ...
                    <div class="reporting-explorer__field reporting-explorer__field--action">
                        ...
                        <button type="button" class="btn btn-primary reporting-explorer__export" wire:click="exportExplorerExcel">
                            Exporter Excel
                        </button>
                    </div>
                </div>
