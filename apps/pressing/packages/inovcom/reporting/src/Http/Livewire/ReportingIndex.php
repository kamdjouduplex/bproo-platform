...
class ReportingIndex extends Component
{
    ...
    /**
     * @return array<int, array{label: string, value: string, status: string|null, trend: float|null}>
     */
    private function buildSummaryCards(array $financial, array $healthIndicators, string $currency): array
    {
        $cards = [];
        $cards[] = ['label' => 'CA Vente Direct', 'value' => fmt_money($financial['pos_sales']) . ' ' . $currency, 'status' => null, 'trend' => null];
        ...
        $cards[] = ['label' => 'Dépenses', 'value' => fmt_money($financial['expenses']) . ' ' . $currency, 'status' => null, 'trend' => null];
        $cards[] = ['label' => 'Pertes', 'value' => fmt_money($financial['losses']) . ' ' . $currency, 'status' => null, 'trend' => null];
        if ($financial['payroll'] != 0) {
            $cards[] = ['label' => 'Paie', 'value' => fmt_money($financial['payroll']) . ' ' . $currency, 'status' => null, 'trend' => null];
        }
        if ($financial['purchases'] != 0) {
            $cards[] = ['label' => 'Achats', 'value' => fmt_money($financial['purchases']) . ' ' . $currency, 'status' => null, 'trend' => null];
        }
