...
class StockService
{
    ...
    public function getStockLevel(int $itemId, ?int $storeId = null): StockLevel
    {
        ...
        $query = StockLevel::query()->where('item_id', $itemId);
        if ($hasStore && $storeId !== null) {
            $match = (clone $query)->where('store_id', $storeId)->first();
            if ($match) {
                return $match;
            }
        }

        // Legacy rows may exist without store_id while a unique(item_id) constraint remains.
        $existing = $query->first();
        if ($existing) {
            if ($hasStore && $storeId !== null && (int) $existing->store_id !== (int) $storeId) {
        ...
        try {
            return StockLevel::create(array_filter([
                'item_id' => $itemId,
                'store_id' => $hasStore ? $storeId : null,
                'quantity' => 0,
                'reserved_quantity' => 0,
