...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="app-body-pharma">
        ...
        @endphp
        <div class="app-shell {{ $isTenantApp ? ($isCashierView ? 'app-shell--cashier' : 'app-shell--sidebar app-shell--pharma') : '' }}" @if($isTenantApp && !$isCashierView) x-data="{ sidebarOpen: false }" @endif>
                ...
                <div class="app-header-inner {{ $isTenantApp && $tenant ? 'app-header-inner--tenant' : '' }}">
                    @if ($isTenantApp && $tenant)
                        <div class="app-header-left">
                            ...
                            @endif
                            <a href="{{ route('tenant.dashboard', ['tenant' => $tenantCode]) }}" class="app-logo-link app-logo-link--tenant">
                                @if ($tenantLogoUrl)
                                    <img src="{{ $tenantLogoUrl }}" alt="{{ $shopName }}" class="tenant-logo-img app-logo-img" @configuration-updated.window="location.reload()">
                                @else
                                    <div class="app-logo app-logo--shop" x-data="{ logoText: '{{ $shopName }}' }" x-text="logoText" @configuration-updated.window="logoText = $event.detail.shopName"></div>
                                @endif
                            </a>
                            @if ($isCashierView)
                    ...
                    @else
                        <div class="app-brand">
                            <a href="{{ route('landing') }}" class="app-logo-link">
                                <div class="app-logo">{{ $appName }}</div>
                            </a>
                        </div>
                        <div class="app-actions">
                            <a href="{{ route('demo') }}" class="btn btn-secondary">Démo</a>
                        </div>
                    @endif
                </div>
            </header>
