...
<head>
    <meta charset="UTF-8">
    <title>{{ $title ?? 'Dettes clients' }}</title>
    <style>
        @page { margin: 18px 20px 26px 20px; }
        body { font-family: DejaVu Sans, sans-serif; color: #111827; font-size: 9.5px; margin: 0; }
        ...
        th { background: #0f766e; color: #fff; font-size: 8px; text-transform: uppercase; }
            ...
            <div class="doc-box">
                <table>
                    ...
                    <tr>
                        ...
                        <td class="doc-title">Dettes clients</td>
                    </tr>
                </table>
            </div>
        </td>
            ...
            <tr>
                ...
                <td>
                    ...
                    @if (!empty($row['client_code']))
                        <div class="muted">{{ $row['client_code'] }}</div>
                    @endif
                </td>
                <td>{{ $row['opened_at'] }}</td>
                <td>{{ $row['due_date'] }}</td>
                <td class="right">{{ number_format((float) $row['total_amount'], 0, ',', ' ') }}</td>
                <td class="right"><strong>{{ number_format((float) $row['balance'], 0, ',', ' ') }}</strong></td>
                <td>{{ $row['status_label'] ?? ($statusLabels[$row['status']] ?? $row['status']) }}</td>
            </tr>
        @empty
            <tr><td colspan="7">Aucune dette pour ces filtres.</td></tr>
        @endforelse
    </tbody>
</table>
