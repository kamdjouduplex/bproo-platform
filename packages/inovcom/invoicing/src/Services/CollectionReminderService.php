...
class CollectionReminderService
{
    ...
    public function groupedOverdueInvoices(array $filters = []): Collection
    {
        ...
        $rows = $invoices->map(function (Invoice $invoice) use ($today, $filters) {
            ...
            // Facture échelonnée : due dès qu'une tranche non soldée est échue.
            if ($invoice->relationLoaded('schedules') && $invoice->schedules->isNotEmpty()) {
                $overdueInstallment = $invoice->schedules
                    ->filter(function ($s) use ($today) {
                        return $s->status !== 'paid'
                            && ((float) $s->amount_due - (float) $s->amount_paid) > 0.01
                            && Carbon::parse($s->due_date)->startOfDay()->lt($today);
                    })
                    ->sortBy('due_date')
                    ->first();
            ...
            $amountDueNow = null;
            if ($invoice->relationLoaded('schedules') && $invoice->schedules->isNotEmpty()) {
                $amountDueNow = round((float) $invoice->schedules
                    ->filter(function ($s) use ($today) {
                        return $s->status !== 'paid'
                            && ((float) $s->amount_due - (float) $s->amount_paid) > 0.01
                            && Carbon::parse($s->due_date)->startOfDay()->lte($today);
                    })
                    ->sum(fn ($s) => max(0, (float) $s->amount_due - (float) $s->amount_paid)), 2);
            }
        ...
        return $rows
            ...
            ->map(function (Collection $clientRows) {
                ...
                $invoiceRows = $clientRows->map(fn ($r) => [
                    ...
                    'due_date' => $r['due_date'],
                    'amount_due_now' => $r['amount_due_now'] ?? null,
                ])->values();
